...
相关项目（非本项目工作项）：context-pilot（context 管理工具，设计笔记见 `~/wiki/pages/ccm_context-pilot-design.md`，2026-07-15 续议已收敛）——与本项目共享 checkpoint schema 与 hook 注入骨架，但稀缺资源/采样层/唤醒向量均不同，独立立项；唯一集成点是 ④ 的 phase 边界 patch-anchor，未来可在同一锚点同时挂额度门与正交门。

与本项目的关键非对称（07-15 收敛，指导双门共存设计）：

- **命题反转**：quota-pilot 是「主动式·会话不死」（冬眠等重置）；context-pilot 是「主动销毁 context」（失忆）。`clear && 继续 X` 在 LLM 语言层不可发音（说出它的 context 被它自己销毁），只能拆帧为 clear 前落盘 / clear 由 harness 执行 / clear 后 SessionStart hook 回注——插件本质是这条不可发音指令的绕行编译器。
- **spawn 角色相反**：quota-pilot 的后台任务是**哑闹钟**（零 payload，为长寿优化：熬过数小时、孤儿恢复、jitter）；context-pilot 的信使**携带 to-do**（为过境保真优化：熬过一次清空、原样重入）。时间尺度数天 vs 数秒，本项目的闹钟基建在彼处整套用不上。
- **信使不能是进程**（直接复用本项目产物）：活着的 spawn 进程只能把返回值塞进没被 clear 的父 context，故真失忆场景的信使只能是**磁盘 checkpoint + SessionStart hook**。其 hook 骨架 = **v0.3.0 `quota_recover.sh` 的孪生**（动态目标 SessionStart 注入器），仅把「孤儿判定」换成「一次性 handoff token 消费」以防悬空 checkpoint 重注入。
- **否决权非对称**：本项目 checkpoint 是保险（coordinator 反正记得），自足性测试挂了只是轻伤；context-pilot checkpoint 是清空后的全部记忆，测不过必须**拒绝清空**——正交门带否决权，额度门没有。
- **立项前置 gate**（对称本项目 V1）——**已验证：确证无（2026-07-15，claude.exe 2.1.206 二进制/派发代码分析）**。三通道全否：①hook 输出 schema 无 clear 动作字段（只有 additionalContext/decision/continue/stopReason/systemMessage/suppressOutput），只能注入或阻断不能清空；②`/clear` 是 `type:"local-jsx"` 命令，仅在交互输入 `onSubmit`（prompt 以 `/` 开头）派发→`clearContext/clearConversation`，模型输出文本 `/clear` 不进 onSubmit；③SDK/headless 的 `control_request` 子类型仅 interrupt/can_use_tool/set_permission_mode/mcp_message，无 clear。**此结论只针对「模型/SDK 能否自己触发 clear」= 不能，须人按**。投递端（clear 后回注）由 2026-07-16 哨兵实测坐实为**自动可用**（下条）。**推论**：context-pilot 触发端无纯原生自动路径——clear 只能人按（交互 TUI 或 happy app remote /clear）；long-run 全自动只能 headless 转生或 opt-in TTY 注入。~~早期「happy 无法 clear / B TUI-only」的判断在 2026-07-16 被实测推翻~~（见下条）。
- **收敛为 B-only**（07-15 定论）：三条唤醒路径中，A（spawn 冷 subagent）本质是 delegation、已被 subagent 覆盖（coordinator 从不真忘），C（headless 转生）是范畴错误——它丢掉 context-pilot 服务的介质「人类可见/可介入/可治理的交互会话」，人在场则有害（TUI 成空壳、permission context 归零＝无人能拦的 agent 在无人看守时执行），人不在场则自败（本就该从头 headless 编排）；两头都有更优解压死 C（交互侧 auto-compact、自动侧 headless 编排）。**只有 B 是 context-pilot 独有**：在人类持续可见/可治理的会话身份上跨过一次无损 context 清空。「pilot」要求一个有人的驾驶舱。
- **投递向量实测坐实**（2026-07-16，SessionStart 哨兵 hook）：stock Claude /clear → `SessionStart(source=clear)`（立即、早于消息），happy app /clear → `SessionStart(source=startup)`，**两环境都 fire、都轮换 session id**（「clear 换 id」是 stock 本身行为，非 happy 特有）。故 B 的投递用 **source 无关的 SessionStart hook + handoff 文件新鲜度护栏 + consume-once**（骨架 fork 本项目 `quota_recover.sh`），stock 和 happy 都零触摸自动。**修正**：早期「happy 是 B 弱环 / 正交门 TUI-only」被推翻——**happy 是一等公民**，正交门在交互 TUI 与 happy 都有效；仅纯无人 headless/SDK 场景该门缺席，只剩额度门。
